...
const Category = () => {
  ...
  const handleDelete = async (idLoaiSanPham) => {
    try {
      await axios.delete(`http://localhost:8000/loaisanpham/${idLoaiSanPham}`);
      setDeletedItemId(idLoaiSanPham);
    } catch (error) {
  ...
  const filteredCategories = categories.filter((service) => {
    return service.tenLoai.toLowerCase().includes(searchValue.toLowerCase());
  });
  return (
    <div id="wrapper">
      {/* Sidebar */}
                      ...
                      {filteredCategories.map((service, index) => {
                        return (
                          <tr key={index}>
                            ...
                            <td className="col-6">{service.tenLoai}</td>
                            <td className="d-flex align-items-center justify-content-between">
                              <Link
                                to={`/updatecategory/${service.idLoaiSanPham}`}
                              ...
                              <button
                                ...
                                onClick={() =>
                                  handleDelete(service.idLoaiSanPham)
                                }
                              >
                                Xóa
                              </button>
